...
import (
	...
	"github.com/buraksezer/olric/internal/transaction"
	"golang.org/x/sync/errgroup"
)

// OlricTransactionServer represents a new olric-control-server instance.
type OlricTransactionServer struct {
	log         *log.Logger
...
func New(c *config.Config, lg *log.Logger) (*OlricTransactionServer, error) {
	tr, err := transaction.New(c, lg)
	if err != nil {
		return nil, err
	}
	return &OlricTransactionServer{
		config:      c,
		log:         lg,
		transaction: tr,
	}, nil
...
func (s *OlricTransactionServer) waitForInterrupt() {
	...
	s.errGr.Go(func() error {
		...
		if err := s.transaction.Shutdown(ctx); err != nil {
...
func (s *OlricTransactionServer) Start() error {
...
func (s *OlricTransactionServer) Shutdown(ctx context.Context) error {
	return s.transaction.Shutdown(ctx)
}
