...
func usage() {
	var msg = `Usage: olric-sequencer [options] ...

Sequencer component of Olric

Options:
  -h, --help    Print this message and exit.
  -v, --version Print the version number and exit.
  -c, --config  Sets configuration file path. Default is olric-sequencer-local.yaml in the
                current folder. Set OLRIC_SEQUENCER_CONFIG to overwrite it.
...
func main() {
	...
	if args.version {
		_, _ = fmt.Fprintf(os.Stderr, "olric-sequencer version %s %s %s/%s\n",
	...
	sq, err := sequencer.New(c, lg)
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "[ERROR] Failed to load the configuration: %v\n", err)
		os.Exit(1)
	}

	p, err := process.New(sq)
	if err != nil {
		lg.Fatalf("[ERROR] Failed to create a new %s instance: %v", sq.Name(), err)
	}

	if err = p.Start(); err != nil {
		lg.Printf("[ERROR] Failed to start %s: %v", sq.Name(), err)
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		if err := p.Shutdown(ctx); err != nil {
			lg.Printf("[ERROR] Failed to shutdown %s: %v", sq.Name(), err)
		}
