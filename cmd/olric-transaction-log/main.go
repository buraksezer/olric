...
func usage() {
	var msg = `Usage: olric-transaction-log [options] ...

Transaction log component for Olric

Options:
  -h, --help    Print this message and exit.
  -v, --version Print the version number and exit.
  -c, --config  Sets configuration file path. Default is olric-transaction-log-local.yaml in the
                current folder. Set OLRIC_TRANSACTION_LOG_CONFIG to overwrite it.
...
func main() {
	...
	if args.version {
		_, _ = fmt.Fprintf(os.Stderr, "olric-transaction-log version %s %s %s/%s\n",
	...
	rs, err := transactionlog.New(c, lg)
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "[ERROR] Failed to load the configuration: %v\n", err)
		os.Exit(1)
	}
